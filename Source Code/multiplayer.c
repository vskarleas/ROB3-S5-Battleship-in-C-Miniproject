...
int multiplayer(int **prop1, int **prop2, Liste_Navire liste1, Liste_Navire liste2, int *NbNav1, int *NbNav2, int *NbJoue_global, int coulle1, int coulle2, int taille_plateau_jeu, int number_of_navires_jeu, int language)
{
    ...
    {
        rules_interface(max_rounds_multi, taille_plateau_jeu, language, 2); ////mode game MULTIPLAYER = 2
        msleep(100);
        waitForKeypress();
        waitForKeypress();
        clearScreen();
        ...
        while (repeat_multi)
        {
            ...
            if (((*NbJoue_global - 1) / 2) >= max_rounds_multi && (*NbNav1 < number_of_navires_jeu || *NbNav2 < number_of_navires_jeu))
            {
                repeat_multi = false;
    ...
    else if (mode_multi == 1) // creating custom plates
    {
        ...
        rules_interface(max_rounds_multi, taille_plateau_jeu, language, 2); //mode game MU:TIPLAYER = 2
        msleep(100);
        waitForKeypress();
        waitForKeypress();
        clearScreen();
