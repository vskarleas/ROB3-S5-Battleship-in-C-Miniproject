...
int main(int argc, char **argv)
{
    ...
    while (repeater)
    {
        ...
        if (mode == LOAD)
        {
            clearScreen();
            load(language);
            repeater = keep_playing(language);
        }
        else
        {
            clearScreen();
            ...
            {
            case MULTIPLAYER:
                multiplayer(prop1, prop2, liste1, liste2, NbNav1, NbNav2, NbJoue_global, coulle1, coulle2, taille_plateau_jeu, number_of_navires_jeu, language);
                repeater = keep_playing(language);
                break;

            case AI:
                ai(prop1, prop2, liste1, liste2, NbNav1, NbNav2, NbJoue_global, coulle1, coulle2, taille_plateau_jeu, number_of_navires_jeu, language);
                repeater = keep_playing(language);
                break;

            case COMPUTER:
                solo(prop1, liste1, NbNav1, NbJoue, coulle1, taille_plateau_jeu, number_of_navires_jeu, language);
                repeater = keep_playing(language);
                break;
            default:
                break;
