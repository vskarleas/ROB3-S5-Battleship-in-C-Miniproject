    ...
    */
    ...
    if (mode==MULTIPLAYER){
        ...
        */
        ...
        rules_interface(ROUNDS, taille_plateau_multi);
        msleep(100);
        waitForKeypress();
        waitForKeypress();
        clearScreen();
        rules_reminder(ROUNDS, taille_plateau_multi);
        ...
        while (taille_plateau_multi)
        {
            ...
            if (waitForMenuKeypress())
            {
                if (midle_game_menu(ROUNDS, taille_plateau_multi, 2, MULTIPLAYER) == 1) // 1 is internal code foe saving the progress and continuing another time
                {
            ...
            else
            {
                ...
                // decision making if the user wins or loses the game
                if (round1 == ROUNDS && *NbNav1 < number_of_navires_multi)
                {
                    repeat_multi = false;
                    lost_graphics();
                    return 1; // returns 1 if the user ran out of rounds - it also works as the while(repeat) stopper
    ...
    if (mode==LOAD){
        ...
        rules_interface(ROUNDS, taille_plateau_load);
        msleep(100);
        waitForKeypress();
        waitForKeypress();
        clearScreen();
        rules_reminder(ROUNDS, taille_plateau_load);
        ...
        while (repeat_load)
        {
            ...
            else
            {
                ...
                // decision making if the user wins or loses the game
                if (round_load == ROUNDS && *NbNav_load < number_of_navires_load)
                {
                    repeat_load = false;
                    lost_graphics();
                    return 1; // returns 1 if the user ran out of rounds - it also works as the while(repeat) stopper
